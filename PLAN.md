...
| **10 — Occupants & expenses** ✅ *(built 13 Sep 2026)* | Occupants per unit (move-in/out, company); expenses per unit with categories, equal/custom split between the occupants present on the bill date, per-person settlement, documents; Expenses dashboard (monthly trend, by property, by unit, by category, outstanding) and per-unit charts; new capabilities per role; Admin user deletion (soft); search box + filters on every list (server-side `q` on paged lists, local filter on detail tables), occupant search, richer global search (units by building, contracts by tenant, occupants, expenses); Operations lands on Units instead of a forbidden dashboard | Bill split adds up exactly; dashboard totals reconcile with the list; link crawl per role finds no dead links | 1 wk |
